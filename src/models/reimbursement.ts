export class Reimbursement {
    ...
    resolver: number; // foreign key -> User, not null
...
export class FullReimbursement {
...
export class MessyReimbursement {

    reimbursementId: number; // primary key
    author: number; // foreign key -> User, not null
    amount: number; // not null
    dateSubmitted: number | string; // not null
    dateResolved: number | string; // not null
    description: string; // not null
    resolver: number; // foreign key -> User, not null
    status: number; // foreign key -> ReimbursementStatus, not null
    type: number; //foreign key -> ReimbursementType
    
    constructor(reimbursementId: number, author: number, amount: number, dateSubmitted: number | string, dateResolved: number | string, description: string, resolver: number, status: number, type: number) {
        this.reimbursementId = reimbursementId;
        this.author = author;
        this.amount = amount;
        this.dateSubmitted = dateSubmitted;
